...
@Author : https://github.com/songyuanping
@Site : 
@File : Smooth_AP.py
@Software: PyCharm
@Brief: 完成的主要功能是:对于网络产生的输出计算出average precision损失
"""
###################### LIBRARIES #################################################
import warnings
...
def loss_select(loss, opt, to_optim):
    ...
    if loss == 'smoothap':
        loss_params = {'anneal': opt.sigmoid_temperature, 'batch_size': opt.bs,
                       "num_id": int(opt.bs / opt.samples_per_class), 'feat_dims': opt.embed_dim}
...
class SmoothAP(torch.nn.Module):
    ...
    def __init__(self, anneal=0.01, batch_imgs=128, num_id=8, feat_dims=64):
        """
        Parameters
        ----------
        anneal : float
    ...
    def forward(self, preds):
        ...
        self.batch_imgs = preds.shape[0]

        # ------ differentiable ranking of all retrieval set ------
        # compute the mask which ignores the relevance score of the query to itself
        mask = 1.0 - torch.eye(self.batch_imgs)  # [100,100]
        mask = mask.unsqueeze(dim=0).repeat(self.batch_imgs, 1, 1)  # [100,100,100]
        # compute the relevance scores via cosine similarity of the CNN-produced embedding vectors
        sim_all = compute_aff(preds)  # [100,100]
        sim_all_repeat = sim_all.unsqueeze(dim=1).repeat(1, self.batch_imgs, 1)  # [100,100,100]
        # compute the difference matrix
        sim_diff = sim_all_repeat - sim_all_repeat.permute(0, 2, 1)  # 计算混淆矩阵
        # pass through the sigmoid
        sim_sg = sigmoid(sim_diff, temp=self.anneal) * mask.cuda()  # [100,100,100]
        # compute the rankings
        sim_all_rk = torch.sum(sim_sg, dim=-1) + 1  # [100,100]

        # ------ differentiable ranking of only positive set in retrieval set ------
        # compute the mask which only gives non-zero weights to the positive set
        xs = preds.view(self.num_id, int(self.batch_imgs / self.num_id), self.feat_dims)  # [5,20,256]
        pos_mask = 1.0 - torch.eye(int(self.batch_imgs / self.num_id))  # [20,20]
        pos_mask = pos_mask.unsqueeze(dim=0).unsqueeze(dim=0).repeat(self.num_id, int(self.batch_imgs / self.num_id), 1,
                                                                     1)  # [5,20,20,20]
        # compute the relevance scores
        sim_pos = torch.bmm(xs, xs.permute(0, 2, 1))  # [5,20,20]
        sim_pos_repeat = sim_pos.unsqueeze(dim=2).repeat(1, 1, int(self.batch_imgs / self.num_id), 1)  # [5,20,20,20]
        # compute the difference matrix
        sim_pos_diff = sim_pos_repeat - sim_pos_repeat.permute(0, 1, 3, 2)  # [5,20,20,20]
        # pass through the sigmoid
        sim_pos_sg = sigmoid(sim_pos_diff, temp=self.anneal) * pos_mask.cuda()  # [5,20,20,20]
        # compute the rankings of the positive set
        sim_pos_rk = torch.sum(sim_pos_sg, dim=-1) + 1  # [5,20,20]

        # sum the values of the Smooth-AP for all instances in the mini-batch
        ap = torch.zeros(1).cuda()
        group = int(self.batch_imgs / self.num_id)  # [100/5]
        for ind in range(self.num_id):
            pos_divide = torch.sum(
                sim_pos_rk[ind] / (sim_all_rk[(ind * group):((ind + 1) * group), (ind * group):((ind + 1) * group)]))
            ap = ap + ((pos_divide / group) / self.batch_imgs)
...
class TripletLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        # print("dist_an:", dist_an,"dist_ap:",dist_ap,"y:",y)
        return self.ranking_loss(dist_an, dist_ap, y)
if __name__ == '__main__':
    input = torch.randn(256, 256, requires_grad=True).cuda()
    for i in range(1,100):
        loss = SmoothAP(i/100, 256, 2, 256)
        output = loss(input)
