...
def prepare_features(file='model_best.pth.tar'):
    ...
    sketchy_zero_ext = SketchyDataset(root_dir=args.root_dir, split='zero', version='all_photo',
                                      zero_version=args.zero_version, transform=transformations, aug=False)

    zero_loader_ext = DataLoader(dataset=sketchy_zero_ext, batch_size=args.batch_size, shuffle=False, num_workers=1)

    sketchy_zero = SketchyDataset(root_dir=args.root_dir, split='zero', zero_version=args.zero_version,
                                  transform=transformations, aug=False)
    zero_loader = DataLoader(dataset=sketchy_zero, batch_size=args.batch_size, shuffle=False, num_workers=1)
