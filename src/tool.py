...
class SupConLoss(nn.Module):
    ...
    def forward(self, features, labels=None, mask=None):
        ...
        # 114,2,228
        # print("batch_size:", batch_size, "anchor_count:", anchor_count, "logits_mask.shape:", logits_mask.shape)
...
@torch.no_grad()
def validate_paired(val_loader, model, criterion, criterion_kd, gcn_model, vec_matrix, test_num=550):
    """
    validate when training
    """
    ...
    for i, (sketch, image, target) in enumerate(val_loader):
        ...
        # compute output
        # with torch.no_grad():
        output_kd, hash_code, features, features_map = model(torch.cat([sketch, image], 0),
    ...
    print(' * mAP@all {mAP:.4f}'.format(mAP=mAP))
...
class MemoryStore(object):
    ...
    # 改进函数: get_class_center
    def get_class_center(self, label):
        if self.memory_full_flag[label] == 0:
            return
        else:
            embed_center = torch.mean(self.memory_embeds[label][:self.memory_full_flag[label], :], 0)
            score, idx = torch.max(F.cosine_similarity(self.memory_embeds[label][:self.memory_full_flag[label], :],
                                                       embed_center.view(1, -1)).view(-1, 1), 0)
            # print("get_class_center idx:",idx)
            return self.memory_embeds[label][:self.memory_full_flag[label], :][idx]
        ...
        for idx, embed in enumerate(embeds):
            ...
            if saved_embed_nums < self.topk:
                ...
                if corresponding_idx.size(0) == 0:
                    ...
                    # 改进
                    query_center = torch.mean(queries[corresponding_idx], 0)
                    # print("query_center.shape: ", query_center.shape, query_center)
                    # print(F.cosine_similarity(queries[corresponding_idx], query_center.view(1, -1), dim=-1))
                    max_score, max_idx = torch.max(
                        F.cosine_similarity(queries[corresponding_idx], query_center.view(1, -1), dim=-1).view(-1, 1),
                        0)
                    query = queries[corresponding_idx][max_idx]
                    # print("max_idx:", max_idx, "query.shape: ", query.shape, query)

                score, furthest_idx = self.get_furthest_entry(query, labels[idx])
                # print("score: ", score.shape,score, "furthest_idx:",furthest_idx.shape,furthest_idx)
                # print("F.cosine_similarity(query.view(1, -1), embed.view(1, -1)):",F.cosine_similarity(query.view(1, -1), embed.view(1, -1)))
                # 计算并更新sketch的embed
                if F.cosine_similarity(query.view(1, -1), embed.view(1, -1)) < score:
                    self.memory_embeds[labels[idx], furthest_idx] = embed
    ...
    def get_furthest_entry(self, query, label):
        ...
        """
        # print("self.memory_embeds[label].shape:", self.memory_embeds[label].shape)
        scores = F.cosine_similarity(self.memory_embeds[label], query.view(1, -1), dim=-1)
        # 改进
        # scores = torch.mm(self.memory_embeds[label].squeeze(), query.view(-1, 1)).squeeze()
        # scores = -(query.view(1, -1) - self.memory_embeds[label].squeeze()).square().sum(-1)
        # print("scores:", scores.shape, scores)
        score, idx = torch.min(scores, -1)
        # print("score:", score, "idx:", idx)
        return score, idx

    # 该loss基于eucliance distance
    def memory_loss(self, queries, labels):
        ...
        for idx, query in enumerate(queries):
            ...
            if center is not None:
                ...
                loss.append(torch.square(query.view(1, -1) - center.view(1, -1)).sum(-1))
        if len(loss) == 0:
            # return torch.Tensor([1.0]).cuda()  # to make the loss invalid
            return torch.Tensor([0.0]).cuda()  # to make the loss invalid
        return sum(loss) / len(loss)

    # 该loss基于cosine similarity
    # def memory_loss(self, queries, labels):
    #     loss = []
    #     for idx, query in enumerate(queries):
    #         center = self.get_class_center(labels[idx])
