...
def main():
    ...
    sys.stdout = Logger(os.path.join(savedir, time.strftime('test-%Y-%m-%d-%H-%M-%S') + '-log.txt'))
    print(time.strftime('test-%Y-%m-%d-%H-%M-%S'))
    ...
    for idx, item in enumerate(sorted(os.listdir(savedir))):
        if item.endswith(".pth.tar"):
            print('prepare SBIR features using saved model use file:', item)
            predicted_features_gallery, binary_features_gallery, gt_labels_gallery, \
            predicted_features_query, binary_features_query, gt_labels_query, \
            scores, binary_scores = prepare_features(item)
...
def prepare_features(file='model_best.pth.tar'):
    # create model
    ...
    if args.resume_file:
        ...
        resume = os.path.join(savedir, file)
    ...
    tuberlin_zero_ext = TUBerlinDataset(root_dir=args.root_dir, split='zero', version='ImageResized_ready',
                                        zero_version=args.zero_version, transform=transformations, aug=False)

    zero_loader_ext = DataLoader(dataset=tuberlin_zero_ext, batch_size=args.batch_size, shuffle=False, num_workers=1)

    tuberlin_zero = TUBerlinDataset(root_dir=args.root_dir, split='zero', zero_version=args.zero_version,
                                    transform=transformations, aug=False)
    zero_loader = DataLoader(dataset=tuberlin_zero, batch_size=args.batch_size, shuffle=False, num_workers=1)
