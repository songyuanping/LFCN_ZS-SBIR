...
from losses.network import init_weights, one_weights, two_weights
...
class CSENet_hashing(nn.Module):
    ...
    def __init__(self, block, layers, groups, reduction, dropout_p=0.2,
        ...
        # 改进
        self.second_last_linear = nn.Linear(512 * block.expansion, hashing_dim, bias=False)
...
def initialize_pretrained_model_hashing(model, hashing_dim, num_classes, settings):
    ...
    print("old_weight.shape: ", old_weight.shape)
    ...
    print("new_1.shape, new_2.shape: ", new_1.shape, new_2.shape)
