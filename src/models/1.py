...
class MatchERT(nn.Module):
    ...
    def forward(self, src_global, src_local, tgt_global, tgt_local, normalize=False):
        ...
        src_global = src_global.unsqueeze(1) + self.seg_encoder(
            4 * src_local.new_ones((bsize, 1), dtype=torch.long)).permute(0, 2, 1)
        tgt_global = tgt_global.unsqueeze(1) + self.seg_encoder(
            5 * src_local.new_ones((bsize, 1), dtype=torch.long)).permute(0, 2, 1)

        # global features were not used in the final model
        input_feats = torch.cat([cls_embed, src_global, src_local, sep_embed, tgt_global, tgt_local], -1).permute(2, 0,
                                                                                                                  1)
...
class Matcher_Loss(nn.Module):
    def __init__(self, num_global_features: int = 128,
        ...
        self.matcher = Matcher(num_global_features=128,
...
class Triplet_Matcher_Loss(nn.Module):
    def __init__(self, num_global_features: int = 128,
                 num_local_features: int = 128,
                 dropout: float = 0.,
                 detach: bool = False,
                 ...
                 ert_activation: str = 'relu',
                 ert_normalize_before: bool = False):
        super(Triplet_Matcher_Loss, self).__init__()
        self.matcher = Matcher(num_global_features=128,
                               num_local_features=128,
                               dropout=0.,
                               detach=False,
    ...
    def forward(self, feature_maps):
        ...
        anchor_feature_map, pos_features_map, neg_features_map = feature_maps[::3], feature_maps[1::3], feature_maps[
                                                                                                        2::3]
        # print("pos_features_map.shape, neg_features_map.shape:", pos_features_map.shape, neg_features_map.shape)
        pos_logits, _, _ = self.matcher(src_local=anchor_feature_map, tgt_local=pos_features_map)
        neg_logits, _, _ = self.matcher(src_local=anchor_feature_map, tgt_local=neg_features_map)
        logits = torch.cat([pos_logits, neg_logits], 0)
...
if __name__ == '__main__':
    ...
    src_global = torch.randn(100, 2048)
    tgt_local = torch.randn((100, 2048, 7, 7))
    tgt_global = torch.randn(100, 2048)
    matcher = Matcher(num_global_features=2048)
    logits, _, _ = matcher(src_local=src_local, src_global=src_global, tgt_local=tgt_local, tgt_global=tgt_global)
    print(logits.shape)
